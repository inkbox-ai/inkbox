...
## 0.4.19 — Hosted realtime voice config + call-WS observe/intervene frames
...
- **Observe + intervene frames for hosted calls.** When an identity runs on platform-hosted voice, the platform sends observe frames and accepts intervene frames on the one existing per-call WebSocket (the same connection it already opens to your app) — there is no separate control socket. A transport-agnostic JSON codec under `inkbox.phone.realtime` lets your main agent decode what it receives and build what it sends back. Observe: `parse_event(...)` decodes into typed events (`call.started`, `call.answered`, `transcript`, `barge_in`, `model.tool_call`, `consult.requested`, `call.ended`); unknown tags surface as an `unknown` variant so forward-compatibility holds. Intervene builders return the exact wire frame: Python `consult_answer(..)`, `say(text)`, `inject_context(text)`, `approve_tool(tool_call_id)`, `deny_tool(tool_call_id, reason=)`, `update_instructions(instructions)`, `hang_up(reason=)` (TS camelCase `consultAnswer` / `say` / `injectContext` / `approveTool` / `denyTool` / `updateInstructions` / `hangUp`; Rust `inkbox::phone::realtime::intervene::*` returning `serde_json::Value`). The frames ride the call socket, so none of them carry a `call_id`.
