...
## 0.4.19 — Hosted realtime voice config, call-WS observe/intervene frames + call-lifecycle webhook
...
- **Observe + intervene frames for hosted calls.** When an identity runs on platform-hosted voice, the platform sends observe frames and accepts intervene frames on the one existing per-call WebSocket (the same connection it already opens to your app) — there is no separate control socket. A transport-agnostic JSON codec under `inkbox.phone.realtime` lets your main agent decode what it receives and build what it sends back. Observe: `parse_event(...)` decodes into typed events (`call.started`, `call.answered`, `transcript`, `barge_in`, `consult.requested`, `call.ended`); unknown tags surface as an `unknown` variant so forward-compatibility holds. Intervene builders return the exact wire frame: Python `consult_answer(..)`, `say(text)`, `inject_context(text)`, `update_instructions(instructions)`, `hang_up(reason=)` (TS camelCase `consultAnswer` / `say` / `injectContext` / `updateInstructions` / `hangUp`; Rust `inkbox::phone::realtime::intervene::*` returning `serde_json::Value`). There is no tool-approval flow — hosted write tools apply immediately. Observe frames carry the `call_id` they belong to; the outbound intervene frames omit it, since that socket is already scoped to one call.
- **`call.ended` webhook.** A new post-call lifecycle webhook event so an agent that is not holding the supervisor WebSocket still learns when a hosted call ends. It is an **agent-identity-owned** subscription (`event_types=["call.ended"]`, `agent_identity_id`), delivered as the standard signed `{id, event_type, timestamp, data}` envelope, and is **fire-and-forget + replayable** (contrast the synchronous `phone.incoming_call` control-plane callback). The payload carries the call (`WebhookPhoneCall` — `PhoneCallResponse` minus `is_blocked`, plus `use_inkbox_agent` and derived `duration_seconds`), resolved `contacts` / `agent_identities`, and the transcript in two forms: an inline, middle-cut **abridged** block (`data.transcript`, present only for platform-hosted realtime calls; discriminate a turn from the abridgment marker on `"marker" in entry`) and an **always-present** `data.transcript_url` pointing at the authoritative verbatim transcript (`GET /phone/calls/{id}/transcripts`, needs an admin API key). New receiver types: Python `CallEndedWebhookPayload` / `CallEndedWebhookData` / `WebhookPhoneCall` / `WebhookCallTranscript` / `CallLifecycleWebhookEventType` / `CallOriginWire`; matching TS interfaces + literals; Rust `CallEndedWebhookPayload` / `CallEndedWebhookData` / `WebhookPhoneCall` / `WebhookCallTranscript` / `CallLifecycleWebhookEventType` / `CallOriginWire`. `verify_webhook` is unchanged (same HMAC).
