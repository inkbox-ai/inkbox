...
- **TypeScript note (source-breaking).** `Contact` gains required properties: `creationSource`, `reviewStatus`, `reviewedAt`, `reviewedBy`, `preferredNameSource`, `preferredNameLockedAt`, `createdByIdentityId`, `mergedIntoContactId`, `isAutoCreated`, `isConfirmed`, and `memoryCount` (plus optional `latestMemory`). Hand-built object literals (fixtures, mocks) need the new properties; parsing defaults them when the wire omits them.
