...
**`data["contact"]` (mail/text) and top-level `contact` (calls):** an optional `{"id", "name"}` address-book match for the remote party — scoped to the identity that owns the receiving mailbox/phone number; `None` when no visible address-book entry matches. Match key: inbound mail → `from_address`, outbound mail → `to_addresses[0]` (CC/BCC-only sends → `None`), text → `remote_phone_number`, inbound call → `remote_phone_number`.
