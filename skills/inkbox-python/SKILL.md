...
Conditional writes: `subscriptions.update(sub.id, event_types=events, expected_revision=sub.revision)` and `subscriptions.delete(sub.id, expected_revision=sub.revision)`. Delivery rows expose `canonical_subscription_id`, `replayable`, and `replay_unavailable_reason` without replacing their original `webhook_subscription_id`.
