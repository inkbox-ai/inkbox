...
- **Post-call actions.** The hosted agent's recorded action items surface inline on the call resource: `PhoneCall` gains `postCallActions` on every call response (`calls.list` / `calls.get` / `calls.hangup` / `calls.place`), `seq`-ascending and open items only. Empty for `client_websocket` calls and hosted calls with no open items. New slim `PostCallAction` type: `{ id, seq, action, details, status }` (`status` always `"open"` on the wire). Mirrors the `call.ended` webhook. No separate endpoint or method.
