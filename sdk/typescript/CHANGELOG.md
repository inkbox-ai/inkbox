...
- `HangupReasonWire` now includes `failed`, `no_answer`, `missed`, `busy`,
  and `dropped`.
- `PhoneCall` gains `endedBy` (`"local"`, `"remote"`, `"unknown"`, or
  `null`) and `providerHangupCause` (raw carrier cause or `null`); the wire
  fields `ended_by` / `provider_hangup_cause` are optional on
  `WebhookPhoneCall` and `PhoneIncomingCallWebhookPayload`. New
  `EndedByWire` export.

## 0.6.8 — Voice AI voice discovery

- Added `inkbox.hostedAgent.listVoices()` returning a typed
  `HostedAgentVoiceCatalog` with `HostedAgentVoiceOption` entries and
  `defaultVoice`. Entries include string IDs, names, descriptions,
  availability, and optional `previewUrl` values; unavailable entries are
  retained.
...
- `webhooks.subscriptions.create(...)` accepts `authToken`, a bearer token
  sent as `Authorization: Bearer <token>` on every delivery and replay
  alongside the signature headers.
- `update(...)` treats `authToken` tri-state: omit to leave it unchanged,
  pass `null` to clear it, or a string to replace it.
- Reads return `WebhookSubscription.authToken` (`null` when unset) plus the
  `hasAuthToken` flag; both default to unset on older servers. Object
  literals of the interface must include the new fields.
...
- Identity create/update use `claimIMessageNumber: true` for atomic dedicated
  line claims.
- Removed `IMessageNumberType`, `IMessageDedicatedNumberType`, and all
  line-type arguments and exports.
...
- `inkbox.a2a.publicDirectory()` and `organizationDirectory()` expose typed,
  searchable Agent Card directories with cursor iterators.
- A2A settings expose `publiclyDiscoverable` and `allowPublicEgress`.
...
- `A2ASettings` literals must add `publiclyDiscoverable` and
  `allowPublicEgress`.
...
- `A2AContext.name` exposes the persisted name, and
  `identity.a2aUpdateContext()` lets either participant rename a shared
  context.

### Changed

- `A2AClient.send()` documents that `contextId` without `taskId` starts a
...
- Direct `A2AContext` object literals must add `name`; received and sent
  context methods retain their original-open meanings.
...
- Authority mode fields on hosted-agent config, calls, and `call.ended`
  webhook types.
- `HostedAgentConfigResource.setAuthorityMode()` and
  `AgentIdentity.setHostedAgentAuthorityMode()` for setting future incoming
...
- **Source-breaking:** identity update options and contact-rule update options no longer accept lifecycle `status`. Remove those properties before upgrading.
...
- Conversation list/detail models expose `groupCreationStatus`; failed creation stays on the same conversation and the next send retries.
- Existing reaction methods support inbound group messages by `messageId`; REST and webhook reaction assignment fields are nullable.
- Reaction types and webhook payloads include the named, sendable `eyes` tapback. Reaction sends reject `custom` and arbitrary emoji before making a request; inbound custom emoji remain available through `customEmoji`.
- Group creation and `conversationId` replies accept the same 13 `IMessageSendStyle` values as one-to-one sends, including sends with one media URL.
