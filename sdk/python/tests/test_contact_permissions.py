...
def test_group_access_preserves_nested_omission_and_atomic_creation_shape() -> None:
    ...
    try:
        ...
        assert client.contacts.access.update(
            ...
            email=ContactChannelAccessUpdate(visible=False, contactable=[]),
            phone=ContactChannelAccessUpdate(visible=False), profile=False, memories=False,
        ) == loaded
        ...
        client.contacts.create(given_name="Ada", permissions=ContactCreatePermissions(
            ...
            email=ContactChannelAccessUpdate(visible=True, contactable=[]), profile=True,
        ))
        with pytest.raises(ValueError, match="Profile cannot be disabled"):
            client.contacts.access.update(
                "test-agent", contact_id, profile=False, memories=True,
            )
        with pytest.raises(ValueError, match="Profile cannot be disabled"):
            client.contacts.permissions.update(
                "test-agent", contact_id, profile=False, emails={"person@example.com": True},
            )
