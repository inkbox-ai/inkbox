...
# ---- VaultResource.delete_key tests ----


class TestVaultResourceDeleteKey:
    def test_derives_auth_hash_and_calls_delete(self):
        res, http = _resource()
        http.get.return_value = VAULT_INFO_DICT

        res.delete_key(VALID_VAULT_KEY)

        http.get.assert_called_once_with("/info")
        http.delete.assert_called_once()

        # The delete path should contain a 64-char hex auth_hash
        import re
        delete_path = http.delete.call_args[0][0]
        assert re.match(r"^/keys/[0-9a-f]{64}$", delete_path)

    def test_auth_hash_in_path_matches_derivation(self):
        res, http = _resource()
        http.get.return_value = VAULT_INFO_DICT
        ...
        salt = derive_salt("org_test_123")
        mk = derive_master_key(VALID_VAULT_KEY, salt)
        expected_hash = compute_auth_hash(mk)

        delete_path = http.delete.call_args[0][0]
        assert delete_path == f"/keys/{expected_hash}"

    def test_works_with_recovery_code(self):
        res, http = _resource()
        http.get.return_value = VAULT_INFO_DICT

        recovery_code = "ABCD-EFGH-JKLM-NPQR-STUV-WXYZ-2345-6789"
        res.delete_key(recovery_code)

        salt = derive_salt("org_test_123")
        mk = derive_master_key(recovery_code, salt)
        expected_hash = compute_auth_hash(mk)
    ...
    def test_different_keys_produce_different_hashes(self):
        res1, http1 = _resource()
        http1.get.return_value = VAULT_INFO_DICT
        res1.delete_key(VALID_VAULT_KEY)
        path1 = http1.delete.call_args[0][0]

        res2, http2 = _resource()
        http2.get.return_value = VAULT_INFO_DICT
        res2.delete_key("Other-Passw0rd!z")
        path2 = http2.delete.call_args[0][0]
