...
- **Post-call actions ride the call object.** The hosted agent's recorded action items (open items only, `seq`-ascending) surface inline on each call in `inkbox phone calls` output — no separate command.
